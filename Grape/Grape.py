...
BasePath = '/Applications/Grape.app/Contents/Resources/'
# BasePath = ''  # test

# Create the icon
icon = QIcon(BasePath + "grape-logo.icns")
...
action3 = QAction("🖼️ Launch Photo Picker!")
menu.addAction(action3)

action4 = QAction("🎵 Launch Music Picker!")
...
btna4 = QAction("&Launch Photo Picker!")
btna5 = QAction("&Launch Music Picker!")
sysmenu = QMenuBar()
...
class window_about(QWidget):  # 增加说明页面(About)
	...
	def setUpMainWindow(self):
		widg1 = QWidget()
		l1 = QLabel(self)
		png = PyQt6.QtGui.QPixmap(BasePath + 'grape-logo.png')  # 调用QtGui.QPixmap方法，打开一个图片，存放在变量png中
	...
	def homepage(self):
		webbrowser.open('https://github.com/Ryan-the-hito/Grape')
...
class window_update(QWidget):  # 增加更新页面（Check for Updates）
	...
	def initUI(self):  # 说明页面内信息
		...
		self.resize(300, 150)
		self.center()
		self.setWindowTitle('Grape Updates')
		self.setFocus()
		self.setWindowFlags(Qt.WindowType.WindowStaysOnTopHint)
	...
	def checkupdate(self):
		targetURL = 'https://github.com/Ryan-the-hito/Grape/releases'
		try:
...
class window3(QWidget):  # 主窗口
	...
	def photopicker(self):
		# main code
		photopick = codecs.open(BasePath + 'Photopicker.txt', 'r', encoding='utf-8').read()
		signal.signal(signal.SIGALRM, self.timeout_handler)
		signal.alarm(60)
		try:
			cmd = """set myCommand to "shortcuts run \\"%s\\""
			do shell script myCommand""" % (photopick)
			...
			self.notify(CMD, "Photo&Music Picker",
		...
		'''
		subprocess.call(['osascript', '-e', action_cmd])
		wait_time = int(codecs.open(BasePath + 'SetTime.txt', 'r', encoding='utf-8').read())
		time.sleep(wait_time)
		shutil.rmtree(self.fulldir1)
		os.mkdir(self.fulldir1)

	def musicpicker(self):
		# main code
		musicpick = codecs.open(BasePath + 'Musicpicker.txt', 'r', encoding='utf-8').read()
		signal.signal(signal.SIGALRM, self.timeout_handler)
		signal.alarm(60)
		try:
			cmd = """set myCommand to "shortcuts run \\"%s\\""
			do shell script myCommand""" % (musicpick)
			subprocess.call(['osascript', '-e', cmd])
		except TimeoutException:
			CMD = '''
			on run argv
...
class window4(QWidget):  # Customization settings
	...
	def setUpMainWindow(self):
		self.lbl1 = QLabel('Wait before clearing cache (seconds): ', self)
		self.le1 = QLineEdit(self)
		self.le1.setPlaceholderText('Seconds. Numbers only, no decimal. Default=10')
		text = codecs.open(BasePath + 'SetTime.txt', 'r', encoding='utf-8').read()
		self.le1.setText(text)

		self.lbl7 = QLabel('Photo picking Shortcut is named as: ', self)
		self.le5 = QLineEdit(self)
		self.le5.setPlaceholderText('Grape Photo Picker')
		High = codecs.open(BasePath + 'Photopicker.txt', 'r', encoding='utf-8').read()
		self.le5.setText(High)

		self.lbl8 = QLabel('Music picking Shortcut is named as: ', self)
		self.le8 = QLineEdit(self)
		self.le8.setPlaceholderText('Grape Music Picker')
		High2 = codecs.open(BasePath + 'Musicpicker.txt', 'r', encoding='utf-8').read()
		self.le8.setText(High2)
	...
	def SetTime(self):
		...
		if self.le5.text() != '':
			...
			with open(BasePath + "Photopicker.txt", 'w', encoding='utf-8') as f0:
				f0.write(High)
		if self.le8.text() != '':
			High2 = str(self.le8.text())
			with open(BasePath + "Musicpicker.txt", 'w', encoding='utf-8') as f0:
				f0.write(High2)
		self.close()
...
if __name__ == '__main__':
	...
	action3.triggered.connect(w3.photopicker)
	action4.triggered.connect(w3.musicpicker)
	action7.triggered.connect(w4.activate)
	btna4.triggered.connect(w3.photopicker)
	btna5.triggered.connect(w3.musicpicker)
